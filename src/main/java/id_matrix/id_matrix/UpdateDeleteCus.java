...
public class UpdateDeleteCus {
    ...
    @FXML
    private ObservableList<Customer> employees;
    UpdateDeleteCus controller;
    @FXML
    ...
    public void onBackButtonClick(ActionEvent event) {
        ...
        displayPage("EmpFunc.fxml", event);
    }
    ...
    public void onDeleteEmployeeButtonClick(ActionEvent event) throws IOException {
        ...
        emailDropdown.setItems(FXCollections.observableArrayList(
                employees.stream().map(Customer::getEmail).collect(Collectors.toList())
        ));
        ...
        // if (deletePane != null)
        {
            ...
            FXMLLoader loader = new FXMLLoader(getClass().getResource("delC.fxml"));
            Parent root = loader.load();

            // Get the current controller
            controller=new UpdateDeleteCus();
...
//         }
        ...
        for (Iterator<User> iterator = idm.users.iterator(); iterator.hasNext(); ) {
            ...
            if (user instanceof Customer && user.getEmail().equals(selectedEmail)) {
        ...
        if (employeeDeleted) {
            ...
            emailDropdown.setItems(FXCollections.observableArrayList(
                    employees.stream().map(Customer::getEmail).toList()
            ));
    ...
    private ObservableList<Customer> getEmployees() {
        System.out.println("Fetching Employees...");
        ObservableList<Customer> employees = FXCollections.observableArrayList();
        ...
        for (User user : idm.users) {
            if (user instanceof Customer) {
                Customer employee = (Customer) user;
